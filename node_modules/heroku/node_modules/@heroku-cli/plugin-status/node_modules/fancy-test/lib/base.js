...
const base = (context) => {
    ...
    return Object.assign({}, Object.entries(context.plugins)
        .reduce((plugins, [k, v]) => {
        plugins[k] = (...args) => {
            const plugin = v(...args);
            // clone context first
            let c = Object.assign({}, context);
            if (plugin.init)
                plugin.init(c);
            return base(Object.assign({}, c, { chain: [...c.chain, plugin] }));
        };
        return plugins;
    }, {}), { register(k, v) {
            return base(Object.assign({}, context, { plugins: Object.assign({}, context.plugins, { [k]: v }) }));
        },
        do(cb) {
            return base(Object.assign({}, context, { chain: [...context.chain, { run: (input) => cb(input) }] }));
        },
        finally(cb) {
            return base(Object.assign({}, context, { chain: [...context.chain, { finally: (input) => cb(input) }] }));
        },
        add(key, v) {
            return base(Object.assign({}, context, { chain: [...context.chain, {
                        run: async (ctx) => {
